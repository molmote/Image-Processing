...
#undef OFFSETOF
	...
	//extern TEAMPROJ_MPR::MPR g_MPR;
	//extern KDTree* spKDTree;
	//extern HWND g_hWnd;
	//extern int g_nDisplayKDMax;
	//extern HBV*	spHBV;
	//HDC	  g_hDC = NULL;
	//HGLRC g_hRC = NULL;


	ImguiManager::ImguiManager(): m_pCamera(nullptr), m_window(nullptr)
	{
		START();
		//m_pCamera = Camera::GetInstance();
		//m_pLightRenderer = new LightRenderer();
		//m_nDepthForDVBH = 0;
		m_nRenderMode = 0;
		//m_window = Application::GetAppInstance()->GetWindow();

		m_p_entitySystem = app->getSystem<VK::EntitySystem>();
		m_p_audioSystem = app->getSystem<VK::AudioSystem>();
		m_p_graphicRenderer = app->getSystem<VK::GraphicsRenderer>();
		m_entityType = 0;
		m_entityList_window = false;
		m_cmpList_window = false;
		m_imguiSwitch = false;
		m_soundEditor_window = false;

		LOG("Initilize Imguimanager");
	}

	ImguiManager::~ImguiManager()
	{
		SAFE_DELETE(m_pCamera);
		//	SAFE_DELETE(m_pLightRenderer);
	...
	void ImguiManager::Init()
	{
		...
		m_window = Application::GetAppInstance()->GetWindow();
		g_Window = Application::GetAppInstance()->GetWindow();

		EntityListVector eList = m_p_entitySystem->GetDynamicEntityList();
		cmpWindowBool.resize(eList.size(), false);
		...
		*/
		...
		//glfwSetKeyCallback(m_window, ImGui_ImplGlfwGL3_KeyCallback);
	...
	{
		for (unsigned i = 0; i < m_vecShapes.size(); i++)
		{
			m_vecShapes[i]->Destroy();
		}
	...
	void ImguiManager::ProcessMessage(Message* message)
	{
		m_pCamera->ProcessMessage(message);

		//if (message->type == TYPE_KEY_INPUT)
		//{
		//	//KeyInputMsg* msg = static_cast<KeyInputMsg*>(message);
		//	/*
		//	if (msg->bIsTriggered && msg->keyCode == 'N')
		//	{
		//		for (unsigned i = 0; i<m_vecShapes.size(); i++)
		//		{
		//			unsigned char normal = (m_vecShapes[i]->GetRenderMode() + 1) % BaseShapeRenderer::TOTAL_DRAW;
		//			m_vecShapes[i]->SetRenderMode(normal);
		//		}
		//	}
		//	else if (msg->bIsTriggered && msg->keyCode == 'B')// Turn on/off drawing of the tree nodes
		//	{
		//		m_bIsTreeNodesRendered = !m_bIsTreeNodesRendered;
		//		printf("Turn %s drawing of the tree nodes\n", ((!m_bIsTreeNodesRendered) ? "Off" : "On"));
		//	}
		//	*/
